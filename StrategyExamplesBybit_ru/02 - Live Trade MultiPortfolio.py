            ...
            if status in [0, 1]:
                ...
                if not self.getposition(data):  # Если позиции нет
                    if order and order.status == bt.Order.Accepted:  # Если заявка на бирже (принята брокером)
                        print(f"\t - Снимаем заявку {order.bybit_order['orderId']} на покупку {data._name}")
                    ...
                    if self.rsi[ticker] < 60:  # Enter long
                        ...
                        print(f"\t - Выставлена заявка {self.orders[data._name].bybit_order['orderId']} на покупку {data._name}")
...
if __name__ == '__main__':
    ...
    symbols = ('BTC', 'ETH', 'BNB')  # тикеры, по которым будем получать данные
